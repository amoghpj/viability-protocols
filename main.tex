...
\begin{itemize}
    ...
    \item Protocol - General qPCR setup (\Cref{sec:qpcr-setup})
    \item Protocol - qPCR setup for bacteria, fungi, and virus (\Cref{sec:qpcr-virus})
...
\begin{itemize}
\item Make the mastermixes for each target in 1.5mL eppendorf tubes. See \Cref{tab:qpcr-calculations} for amounts of each reagent to use. Reach out to any Springer Lab member for the location of more reagents. Make mixes of the LUNA mastermix, RT enzyme and water first to avoid pipetting small volumes.
...
The destination wells on a plate are numbered sequentially down the rows. See \Cref{tab:destinationmap}
